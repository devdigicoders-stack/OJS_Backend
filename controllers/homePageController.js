...
export const getHomePage = async (req, res) => {
  try {
    ...
    if (!page) {
      page = await HomePage.create({ stats: [], domains: [], reviews: [] });
    }
...
export const updateHomePage = async (req, res) => {
  try {
    ...
    if (!page) {
      ...
      page = await HomePage.findByIdAndUpdate(page._id, { $set: data }, { new: true, runValidators: true });
    }
    res.status(200).json(page);
  } catch (error) {
